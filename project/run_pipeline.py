...
def run_pp(
    ...
    train_loader = DataLoader(
        dataset["train"], batch_size=batch_size, shuffle=False, collate_fn=collate_fn
    )
    ...
    for epoch_idx in range(n_epochs):
        ...
        if not PYTEST:
            ...
            print(
                f"Epoch {epoch_idx}: Training Time = {training_time}, Tokens_per_sec = {avg_tokens_per_sec}"
            )
    ...
    if not PYTEST:
        ...
        print(
            f"Training time: avg:{np.mean(total_time)}, std:{np.std(total_time)}, \
