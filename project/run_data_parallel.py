...
def run_dp(
    ...
    train_loader = partition_dataset(
        rank, world_size, dataset["train"], batch_size=batch_size, collate_fn=collate_fn
    )
    ...
    for epoch_idx in range(n_epochs):
        ...
        if not PYTEST:
            ...
            print(
                f"Epoch {epoch_idx} on Rank {rank}: Training Time = {training_time}, Tokens_per_sec = {avg_tokens_per_sec}"
            )
            ...
            print(
                f"Epoch {epoch_idx} on Rank {rank}: Validation Loss = {validation_loss}"
            )
            ...
            json.dump(
                gen_examples,
                open(f"{workdir}/rank{rank}_gen_epoch{epoch_idx}.json", "w"),
                indent=4,
            )
    ...
    if not PYTEST:
        ...
        print(
            f"Rank {rank} training time: avg:{np.mean(total_time)}, std:{np.std(total_time)}, \
