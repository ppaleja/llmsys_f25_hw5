...
def loss_fn(batch, model):
    ...
    return torch.sum(loss * batch["label_token_weights"].reshape(-1)) / torch.sum(
        batch["label_token_weights"]
    )
...
def generate(
    model, examples, src_key, tgt_key, tokenizer, model_max_length, device, desc
):
    model.eval()
...
def train(
    ...
    for i, batch in enumerate(
        prog_bar := tqdm.tqdm(examples, desc=f"Training ({desc})")
    ):
        t0 = time.time()
        optimizer.zero_grad()

        # Ensure batch tensors are placed on the same device as the model.
        # This is defensive: collate_fn should already put tensors on device, but
        # some callers may not. Determine model device from its parameters.
        try:
            model_device = next(model.parameters()).device
        except StopIteration:
            model_device = torch.device("cpu")

        for key in ("input_ids", "labels", "label_token_weights"):
        ...
        loss = torch.sum(loss * batch["label_token_weights"].reshape(-1)) / torch.sum(
            batch["label_token_weights"]
        )
        loss.backward()
        ...
        # Protect against zero division if batch_time is 0 (very unlikely) and guard types.
        if batch_time > 0:
            tokens_per_sec.append(tokens / batch_time)
        else:
            tokens_per_sec.append(0.0)
        tokens_num.append(tokens)
        prog_bar.set_postfix(
            tokens_per_sec=(tokens / batch_time if batch_time > 0 else 0.0),
            loss=loss.item(),
        )

    # Protect from empty tokens_per_sec (e.g., no batches processed)
    if len(tokens_per_sec) == 0:
        return 0.0, tokens_num
